...
typedef struct __pqElement {
	...
	{
		//	return (hu> rhs.hu) || (hu== rhs.hu && distance < rhs.distance);
		return (distance < rhs.distance) || (distance == rhs.distance && hu> rhs.hu);
...
int main(int argc, const char * argv[]) {
	...
	off << "---" << endl;
	off << "length : " << len << endl;
	off << "time : " << retVal << endl;
	off.close();

	cout << len << " " << retVal << endl;
	return 0;
}
